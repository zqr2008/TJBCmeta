...
# Main output(s): instrain_transmission.rda; instrain_transmission_overall.rda; instrain_transmission_overall.csv; Instrain_transmission_spearate_revisionbar_dividedbytotal.pdf; plus 3 more
...
# <<< PUBLISH_ANNOTATION_END
  ...
  metadatatraj <- metadata %>%
    distinct(FID,class_1)
...
for (speciesnumber in levels(factor(inStrain_C_genome$species))){
    for (tt in levels(factor(inStrain_C_genome$class_1))){
      ...
      specieswithnext <- inStrain_C_genome %>%
        filter(str_detect(pair_visit,"MG") & str_detect(pair_visit,"BM") ) %>%
        filter(class_1 == tt) %>%
        filter(species == speciesnumber) %>%
        distinct(FID)
      ...
      instrain_transmission_overall[x,3] <- "MG"
      ...
      print(paste0("overall","MG",speciesnumber))
      x = x + 1
    }
...
transmissibillity_merge <-instrain_transmission_overall %>%
  ...
  #filter(sum(`Transmitted number`, na.rm = TRUE) > 10) %>%
...
ggplot(df_long, aes(x = Species, y = count, fill = fill_group)) +
  ...
  labs(
    ...
    y = "Number of dyads",
    fill = "",
    title = "Strain transmitted proportion by species and trajectory"
  )

ggsave("C:/Users/zqr20/Documents/tjmeta/BIG_revision/05_working_figures/exploratory_figures/Instrain_transmission_spearate_revisionbar_dividedbytotal.pdf",width = 24, height = 9)
